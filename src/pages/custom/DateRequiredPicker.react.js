...
class FormDateRequiredPicker extends React.PureComponent<Props, State> {
  static defaultProps = {
    monthLabels: [
      "January",
      "February",
  ...
  _renderMonths = (): React.Node => {
    ...
    return (
      <Form.Select className="required-date-picker-month" name={this.props.monthName} onChange={onChangeMonths}>
  ...
  _renderYears = (): React.Node => {
    ...
    return (
      <Form.Select className="required-date-picker-year" name={this.props.yearName} onChange={onChangeYears}>
        {yearsRange.map(year => (
          <option value={year}>
            {year}
          </option>
...
export default FormDateRequiredPicker;
